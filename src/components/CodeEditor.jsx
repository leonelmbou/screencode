...
const CodeEditor = () => {
  ...
  useEffect(() => {
    if (store.autoDetectLanguage) {
      ...
      const highlightedLanguage = language.toLowerCase() || "plaintext"; // Stocker la valeur de language dans une variable locale
      useStore.setState({
        language: highlightedLanguage, // Utiliser la variable locale ici
      });
    }
      ...
      >
        <Editor
          ...
          highlight={(code) =>
            hljs.highlight(code, {
              language:
                store.language.toLowerCase() !== "paintext"
                  ? store.language.toLowerCase()
                  : "plaintext", // Utiliser store.language ou "plaintext" si "paintext" est détecté
            }).value
          }
          style={{
